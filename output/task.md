# AI Daily Lab — 2026-01-02

## Task
1. Generate a synthetic binary classification dataset using `sklearn.datasets.make_classification` with at least 1500 samples, 8 features (e.g., `n_informative=6`), and 2 classes (set `random_state` for reproducibility).
...
## Focus
ML pipelines, Model Evaluation, Data Visualization

## Dataset
Synthetic binary classification data generated by `sklearn.datasets.make_classification`.

## Hint
Remember to use the `best_estimator_` attribute from your trained `GridSearchCV` objects to make predictions and plot ROC curves on the test set. Keep the hyperparameter grids small to ensure the task finishes within the time limit. `n_jobs=-1` for GridSearchCV can speed up cross-validation.
