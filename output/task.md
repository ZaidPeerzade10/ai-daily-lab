...
## Focus
Predicting future user churn based on historical session activity, using time-windowed feature engineering and a binary classification ML pipeline.

## Dataset
Synthetic user and session activity data, designed to reflect churn patterns.

## Hint
Pay close attention to the time windows: features are aggregated *before* `feature_cutoff_date`, while the target (`is_churned_future`) is defined by activity (or lack thereof) *between* `feature_cutoff_date` and `global_analysis_date`. This separation is crucial for robust predictive modeling.
